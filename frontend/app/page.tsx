        ...
        <div className="h-screen col-span-1 bg-[#f5f5f4] border-solid border-r-2 black rounded-sm">
          <div className="flex h-screen gap-1 flex-col">
            <div className="p-4 flex-col gap-3 items-center justify-between">
              <h2 className="text-2xl py-6 font-bold font-mono dark:text-[#040404]">My Drives</h2>
          ...
          {Object.entries(files).map(([category, items]) => (
            items.length > 0 && (
              <div key={category} className="mb-6">
                <h3 className="text-xl font-semibold mb-4">{category.charAt(0).toUpperCase() + category.slice(1)}</h3>
                <ul>
                  {items.map((file: any) => (
                    <li key={file.id} className="mb-2 flex items-center gap-2">
